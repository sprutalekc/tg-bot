...
Никогда не предлагай "объяснить подробнее", "разобрать по шагам", "подготовить график" и т.п. — просто делай это или не делай.
...
2. Реши задачу. Простая — коротко. Сложная — пошагово, но без лишнего.
3. Пиши как опытный преподаватель на доске: только ключевые шаги, без пересказа очевидного.
   Симметричные случаи — разбери один, для остального скажи "аналогично".
   Промежуточные вычисления которые студент осилит сам — не расписывай.
4. Если нужно заполнить поля — в конце отдельно перечисли значения.
5. Если фото плохо видно — скажи и попроси лучше.
...
- Никогда не заканчивай фразами "Если есть вопросы — спрашивай", "Не стесняйся", "Удачи!",
  "Если нужно — объясню подробнее", "Если нужно, подготовлю...", "Могу разобрать подробнее".
  Это не твой стиль. Ты не предлагаешь — ты просто отвечаешь.
...
- Экспонента: np.exp(x)
- Модуль одной переменной: np.abs(x)
- e и π: np.e, np.pi
- Только np.*, никаких math.*

МОДУЛЬ ВСЕГО ВЫРАЖЕНИЯ — критически важно:
Если в задаче написано |f(x)| — весь f(x) оборачивается в np.abs():
  |x² - 4|x| + 3|  →  np.abs(x**2 - 4*np.abs(x) + 3)
  Это НЕ то же самое что x**2 - 4*np.abs(x) + 3 (без внешнего abs)!
  Без внешнего np.abs() функция будет отрицательной там где должна быть положительной,
  и график уйдёт вниз вместо того чтобы отразиться вверх.
  Пример: при x=2, x²-4|x|+3 = -1, поэтому без внешнего abs y=-1 (яма),
  а с внешним abs y=1 (горб вверх). Всегда проверяй наличие внешнего модуля.
...
def build_graph(graph_data: dict) -> bytes:
    ...
    # Авто-обрезка по x: обрезаем до области где y в разумных пределах
    if ys and all_y_vals:
        # Находим ylim который уже выставлен
        cur_ymin, cur_ymax = ax.get_ylim()
        y_margin = (cur_ymax - cur_ymin) * 0.5

        # Ищем правую и левую границу где функция ещё видна
        visible_x = []
        for yi in ys:
            visible = np.where(
                np.isfinite(yi) &
                (yi >= cur_ymin - y_margin) &
                (yi <= cur_ymax + y_margin)
            )[0]
            if len(visible):
                visible_x.extend([x[visible[0]], x[visible[-1]]])

        if visible_x:
            span = x_max - x_min
            pad = span * 0.05
            auto_xmin = max(x_min, min(visible_x) - pad)
            auto_xmax = min(x_max, max(visible_x) + pad)
            if auto_xmax - auto_xmin > 1:
